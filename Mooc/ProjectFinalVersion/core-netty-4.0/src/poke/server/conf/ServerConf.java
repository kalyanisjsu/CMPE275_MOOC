...
public class ServerConf {
	...
	private NextNearestConf nextnearest;
	...
	public void addNextNearestNode(NodeDescription node)
	{
		if(nextnearest==null)
		{
			nextnearest=new NextNearestConf();
		}
		nextnearest.add(node);
	...
	public NextNearestConf getNextNearest()
	{
		return nextnearest;
	}

	public void setNextNearest(NextNearestConf nearest)
	{
		this.nextnearest=nearest;
	}
	...
	@XmlAccessorType(XmlAccessType.FIELD)
	public static final class NextNearestConf {
		private TreeMap<String, NodeDescription> nextNearest;

		public NodeDescription getNode(String name) {
			return nextNearest.get(name);
		}
		...
		public void add(NodeDescription node) {
			if (node == null)
				...
				nextNearest = new TreeMap<String, NodeDescription>();

			nextNearest.put(node.getNodeId(), node);
		}

		public TreeMap<String, NodeDescription> getNextNearestNodes() {
			return nextNearest;
		}

		public void setNextNearestNodes(TreeMap<String, NodeDescription> nearest) {
			this.nextNearest = nearest;
		}
	}
